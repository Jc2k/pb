...
| 2026-07-18 | Field follow-up | `fix: stop repeated capped agent actions` | focused capped-native-action fixtures; repository-wide Rust, web, docs, formatting, release, and FlashMoe smoke checks | DeepSeek V4 Flash Typing-of-the-Dead run remained safely gated but exposed one extra unchanged 2,048-token retry; normalized native-action identity now survives an intervening failed parsed action and resets on real fingerprint progress | [DeepSeek V4 Flash field run](benchmarks/deepseek-v4-flash-agent.md); no model-family override or acceptance relaxation |
