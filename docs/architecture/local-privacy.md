...
The hidden Rust semantic and native-world qualifiers have the same local-only boundary: they
materialize checked offline Cargo fixtures in ephemeral workspaces, invoke no model or LSP, execute
no build script, procedural macro, or generated mutation, and emit only native-world/corpus
identities, counts, decisions, timings, and process memory totals. Exhaustive prefix, deterministic
rollback, serialized-overlay stress, and resident-memory probes remain in process.
