...
of duplicating it. Exact worlds are reused across turns and workflow stages. Changes to already
...
boundary and prevents prompt work, budget reservation,
invocation accounting, and model entry. A request waiting behind the same in-flight world load
polls cancellation at 100 ms intervals. The pinned in-process rust-analyzer
loader does not yet offer bounded mid-query cancellation to the request that owns the load, so its
cancellation completion can still wait for the current load/prime operation; pb does not describe
that latency as bounded.
