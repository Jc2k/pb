...
drift during preparation refuses the invocation. Cancellation observed while native preparation is
running is checked again at the readiness boundary and prevents prompt work, budget reservation,
invocation accounting, and model entry. The pinned in-process rust-analyzer loader does not yet
offer bounded mid-query cancellation, so cancellation completion can still wait for the current
load/prime operation; pb does not describe that latency as bounded.
