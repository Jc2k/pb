...
overflow before any child starts. In a multi-Task proposal, Rust also rejects a Task that owns only
decomposition constraints: tests, documentation, ordering, generic validation, and smaller-model
instructions must remain attached to a behavior-owning Task. Planning has three attempts and its
own invocation, token, advisory, and wall-time allowance.
