...
| Native Python v1 constraint | Resolves first-party and newly generated cross-file imports plus selected promoted type shapes against one exact pinned Astral `ty`/Python 3.12/typeshed world. Create/modify/delete candidates are applied as one overlay transaction; closure checks every non-deleted frozen first-party file plus new Python files, including untouched in-project dependants. Only complete literal contradictions and newly introduced promoted diagnostics may veto; generated suppressions cannot bypass the gate. `Any`, dynamic/runtime behavior, dependants outside the frozen project, unqualified external environments, and unpromoted diagnostics remain unknown; this is not a general type-correctness guarantee. |
