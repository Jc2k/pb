...
The hidden Rust and Python semantic qualifiers are local, model-free validation surfaces rather than
new execution authority. They materialize only checked bounded fixtures in ephemeral workspaces, use
the ordinary native analyzers and mutation gates, execute no generated mutation, and emit
content-free identities, decisions, counts, and timings.
