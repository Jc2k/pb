...
empty, mutation-forbidden work. Inline completion exposes only the model-owned step status, summaries
limited to 1,024 characters, a constrained Conventional Commit type, and a bounded non-empty commit
description. Plan identity, fingerprints, touched paths, the no-change fact, and the final subject are
projected from trusted current state only after the mutation succeeds; pb assembles and validates the
subject within its 200-character limit.
