...
- `metal.rs` now owns Metal command labels, status decoding, timeout/failure formatting, and
  ...
  descriptors, KV/linear-state layout validation, Q4 source-buffer reuse, and phase buffer
  lifecycle markers now have a module boundary for the CMD1/CMD2/CMD3 builders to move behind.
