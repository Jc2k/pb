...
- `scheduler.rs` now owns graph-stage resolution, CMD1 resolved input-state handoff, CMD2/CMD3
  ...
  selection, declared router-score batches for CPU routing output, declared CMD1 input-state
  validation, CMD2 routing-output validation, full-attention KV placement and execution-shape
