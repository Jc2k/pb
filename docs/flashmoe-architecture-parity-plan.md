...
- `metal.rs` now owns Metal command labels, status decoding, timeout/failure formatting, and
  ...
  are typed in the Metal module. Legacy still owns the concrete encoders and Obj-C buffer
  lifecycle, but command submission diagnostics, wait policy, attention placement policy, kernel
  definitions, post-attention prep payloads, projection/attention buffer descriptors,
  KV/linear-state layout validation, reusable buffer records, Q4 source-buffer reuse, and phase
