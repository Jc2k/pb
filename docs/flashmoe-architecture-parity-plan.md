...
- Shared experts now enter CMD3 through a scheduler descriptor that carries source and graph shape,
  ...
  Shared-expert temporary buffers and CMD3 hidden/shared/expert/next-normed output buffers now
  materialize as Metal-owned records derived from the command plan; active-expert
  activation/projection buffers and submitted command context also resolve through Metal-owned plan
