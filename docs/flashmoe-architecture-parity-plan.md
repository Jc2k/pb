...
- `experts.rs` now owns fixed-slot metadata, the fixed-slot packed-record test fixture, the runtime
  ...
  validation, native-Q4 consistency checks, and Q4 record byte/group accounting now live in
  `experts` behind a typed manifest-tensor adapter, leaving legacy cache build code to bridge
  safetensor byte decoding for now. PBQ4 remains import/build compatibility; execution reads are
  moving toward fixed whole-expert slots.
