...
coverage, and the TypeScript/JavaScript language crate remain phased design-record work.** This is
...
| Workspace all-target tests | Passed | On 2026-07-28, 1,494 root tests passed with 24 device/environment/qualification tests ignored, 2 environment-contract tests passed, all 66 collar tests passed, all 14 native Python tests passed, and all 8 native Rust tests passed |
...
- **7E — Add `pb-control-python` (initial profile implemented; qualification in progress).** The
  ...
  matrix is implemented. Next, add editable/out-of-project dependency authority, expand each
  promoted code's annotated/unannotated
  differential corpus, and require a separately named monotonic proof before granting any new
  token-time hard-mask authority.
...
| Python `ty` deeper profile and `Unknown` policy | Phase 7E | First-party/typeshed plus bounded safe project-local environment identity, isolated dependency-resource matrices, and an initial six-code positive/unknown/repairable-prefix corpus are implemented. Five diagnostics are closure-only; one literal operator has a named token proof. Larger annotated/unannotated and third-party differential corpora, editable/out-of-project environments, false-rejection audit, and any broader token-time promotion remain |
