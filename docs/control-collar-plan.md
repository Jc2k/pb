...
Status: **The syntax/canonical-patch collar is shipped. The native Rust v2 and Python v1 semantic
tranche is production-qualified for its stated conservative profiles: pre-inference readiness,
streaming steering, transaction-wide closure, independent execution replay, exhaustive prefix and
rollback corpora, and release lifecycle/serialization/resource/reclamation matrices all pass.
Current Qwen llama.cpp runs exercise full-vocabulary probing; wider FlashMoe/DeepSeek adapter parity
is not claimed. TypeScript/JavaScript remains frozen at its shipped syntax level. Deeper
compiler/runtime coverage remains phased design-record work.** This is the durable design, migration
history, and evidence ledger. Curated architecture documentation describes only shipped guarantees.
...
- **Native Rust v2 layer implemented:** `pb-control-rust` directly embeds pinned rust-analyzer
  crates. It loads one project-wide Salsa/HIR world from a verified immutable shadow before an
  inference that can edit Rust, primes local/dependency scopes, selects the exact Cargo target for
  existing files, resolves imports and callable/type shapes through HIR, and gives the decoder only
  a cheap request-local snapshot. An exact no-build-script/no-procedural-macro profile also copies
  the loaded graph, roots, configuration, and VFS inputs into a second independently writable Salsa
  storage during readiness. Complete modifications to indexed sources are applied together at tool
  closure; promoted HIR diagnostic debt is compared across all local target modules and the database
  is restored before another request can use it. It does not ask an LSP for diagnostics on
  incomplete source or rerun Cargo during generation.
- **Native Python layer implemented:** `pb-control-python` directly embeds exact-pinned Astral `ty`
  internals. It copies the controller shadow into a frozen in-memory filesystem, uses Python 3.12
  as a fallback, and binds the host target platform, first-party imports, bundled typeshed, and an
  optional exact project-local virtual-environment image into one semantic identity. Every captured
  dependency module and independently writable Salsa database are primed before each inference request. Request-local
  copy-on-write overlays resolve created, modified, and deleted cross-file imports and callable
  shapes as one transaction. A complete generated string-plus-integer literal can be rejected at a proven
  statement boundary; completed transactions reject only newly introduced promoted `ty`
  diagnostics. Missing absolute external imports remain open unless the environment search space is
  qualified complete; `Any` and unpromoted/dynamic cases remain open. Generated `ty: ignore`/`type:
  ignore` directives cannot suppress the gate, while preserved baseline directives are not
  reclassified as generated.
- **Lifecycle implemented:** exact workspace/configuration/dependency identities key a bounded
  ...
  receive its result through a bounded direct handoff, so unrelated LRU eviction cannot trigger a
  duplicate cold build for an already registered waiter. A cold load runs in a
  request-independent in-process worker under that flight;
  both its initiating request and later waiters poll cancellation every 100 ms. At most one cold
  Rust world and at most one Python world are built at a time, so cancelled requests cannot
  accumulate native analyzer loads. Cancellation abandons the request, not the exact local build,
  which may finish into its bounded cache. Existing `.rs` edits revise the warm Salsa world only
  after all request leases from
  ...
  When both languages are reachable, Rust preparation deliberately completes first, Python second,
  and inference receives no stack until both readiness receipts exist. Mixed-language patches
  finalize every participating layer rather than only the layer for the last streamed file.
...
- **Semantic enforcement implemented conservatively:** exact builtin/sysroot facts and local
  literal contradictions can hard-reject. Rust project-local facts remain repairable until closure;
  the exact profile can then reject increases in promoted name/supported-import, field/method,
  privacy, selected call/type/trait-bound, mutability, and moved-from-reference diagnostic debt.
  Facts affected by disabled build scripts, procedural macros, source-topology changes, unsupported
  relative imports, or incomplete dependency configuration remain `Unknown`. Completed
  writes/edits/replacements/patches are reconstructed and replayed through a fresh analyzer stack
  immediately before executor entry; sampler state is never final authority.
- **Language-owned streaming qualification implemented:** the frozen Python v1 and Rust v2 corpora
  run every mutation through the production generation gate, independent final replay, and a direct
  content-free diagnostic/unknown replay. Every logical UTF-8 payload boundary is probed in order;
  once a prefix rejects, no longer prefix may become viable. Each case also performs 64 deterministic
  rollback/full-replay branches. The Rust matrix covers every promoted deep diagnostic, baseline
  debt, cross-crate repair, create/delete topology unknowns, and all four mutation tools. The Python
  matrix covers all six promoted diagnostics, frozen dependencies, dynamic unknowns, cross-file
  repair, and all four tools. Neither qualifier uses an LSP or model, and Python has no Pyright
  runtime dependency.
- **Native release qualification implemented:** version 2 process-isolated Python and Rust matrices
  cross the ordinary cold/warm/process-cache/final-replay barriers on tiny, representative, and large
  dependency graphs. Multiple host workers then alternate invalid and valid mutations against the
  serialized writable analyzer database, require exact decisions plus a final recovery replay, and
  bound both whole-process peak RSS and current resident growth after stress.
...
- **Phase 8 adapter implemented:** llama.cpp now masks the full vocabulary before sampling
  ...
  exact request-local parser, patch, and reusable language-layer batch checkpoints; rollback does
  not allocate another analyzer snapshot per vocabulary entry. Incomplete byte-BPE UTF-8 tails
...
language semantics live in one unpublished crate per language family. The implemented crates are
`pb-control-rust` and `pb-control-python`; a later qualified implementation is expected to be
`pb-control-typescript` rather than a generic runtime "fact pack".
...
pb binary/library
    ...
    `--> pb-control-python
             +-- pinned Astral ty project/Salsa/type integration
             +-- frozen project/typeshed and request-local overlay resolution
             +-- Python-specific streaming boundary certainty
             `-- baseline-debt and final transaction validation
```
...
the exact ready world. Rust v2 can reject exact immutable-dependency contradictions while streaming
and increases in its promoted complete-transaction HIR diagnostic debt after all indexed-source
modifications are overlaid together. Build scripts, procedural macros, source-topology changes,
unsupported relative imports, complete borrow checking, and compiler parity remain `Unknown`.
Python v1 can reject a complete generated string-plus-integer literal operation and selected newly introduced `ty`
import/type diagnostics at transaction closure. `Any`, dynamic imports, monkey-patching,
descriptors, runtime dispatch, and environment facts outside a qualified static dependency image
remain unknown; Python
runtime behavior cannot generally be proven by a static streaming analyzer.
...
```toml
[workspace]
members = [".", "crates/pb-control-collar", "crates/pb-control-python", "crates/pb-control-rust"]
default-members = [".", "crates/pb-control-collar", "crates/pb-control-python", "crates/pb-control-rust"]
resolver = "3"
...
- the root package plus unpublished `pb-control-collar`, `pb-control-rust`, and
  `pb-control-python` crates are explicit workspace/default members using resolver 3; language
  internals remain exact-pinned and private to their owning crates;
...
- Rust-edit-capable real-backend requests prepare a project-wide read database before prompt
  reservation or model-invocation accounting, reuse exact warm worlds, and incrementally refresh
  existing source files only when no request lease can observe the prior Salsa revision. Exact
  profiles also prepare an independently writable database from the same already loaded inputs;
- the Rust stream uses direct HIR scope/import/callable shapes plus a Rust Tree-sitter structural
  stream. Project-local facts remain repairable until closure. Completed modifications to indexed
  files are then overlaid together, all local target modules are queried, and only increases in the
  promoted diagnostic categories reject. Build scripts, procedural macros, topology changes,
  unsupported relative imports, full borrow checking, and compiler parity stay `Unknown`;
...
Rust v2 directly uses rust-analyzer HIR for project target selection, import resolution, public
symbol/callable shapes, conservative literal expression checks, and the promoted finished-overlay
diagnostic allowlist. Later profiles may use compiler-compatible queries for complete traits,
ownership/borrows, `cfg` matrices, and macro-aware checks. Python strictness must be defined relative
to pinned `ty`/Python/project state;
...
4. Revalidate the live workspace after preparation. Drift aborts the current invocation and asks
   the controller to capture a fresh mutation/semantic snapshot; the lifecycle never silently
   rebases the already authorized snapshot.
...
The shipped Rust v2 scheduler uses a synchronous readiness barrier immediately before the first
...
"Rust preparation" in the shipped safe-partial profile means offline Cargo metadata,
rust-analyzer VFS/project loading, dependency/sysroot scope construction, and HIR/Salsa priming; it
does not run `cargo check`, build scripts, or procedural macros and is not a compilation guarantee.
If a later sandboxed-exact profile adds compiler work, its `cargo check`/macro/build-script phase and
artifact-cache lookup must run behind the same pre-inference readiness barrier, keyed by the exact
source/toolchain/configuration/dependency world. Compilation may prewarm during an earlier read-only
stage, but an edit-capable model invocation must await its exact receipt; compilation is never
started lazily from token sampling or final mutation closure.

Cancellation has two distinct guarantees. The shipped lifecycle rechecks cancellation before and after native
preparation, so a cancellation is never followed by prompt work or model inference. Cold project
loading and native query priming run in request-independent in-process threads under per-language
exact-world single-flight leases; initiating and queued requests poll cancellation every 100 ms,
and only one cold Rust plus one cold Python load can run process-wide. A cancelled request therefore stops waiting promptly
...
Rust/Python tranche promotion requires its language, lifecycle, replay, repository, release, and
current full-vocabulary rows below. Backend-specific rows gate only the backend named in that row;
an unrelated FlashMoe checkpoint-quality failure cannot be converted into a collar failure or an
output correction. Each recorded pass covers the exact current workspace, not an earlier phase
branch.

| Evidence | Status | Current result |
| --- | --- | --- |
| Collar unit and chunk-boundary corpus | Passed | 70 deterministic tests, including all six language families plus TSX/JSX, byte/random chunk equivalence, monotonic hard rejection, source-length-only Rust branch rollback, 4,097 sibling-token restores against a 64-checkpoint analyzer arena, transactional rollback after tool closure, mixed-language finalization, viable split-UTF-8 Qwen/DSML boundaries, partial patch-line probing, embedded HTML languages, exact multi-hunk/create/delete patches, malformed patch corpus, controller-bound and scalar-exact payload closure, immutable semantic identities/debt and receipts, and control-token identity |
...
| Workspace all-target tests | Passed | On 2026-07-28, the final serialized build run passed 1,518 root tests with 26 device/environment/qualification tests ignored, 2 environment-contract tests, all 70 collar tests, all 15 native Python tests, and all 13 native Rust tests. The first default-parallel compile was replaced after two root `rustc` jobs slept at 0% CPU without linker children for 15 minutes; `-j 1` completed the same suite with zero failures |
| Web and documentation tests | Passed | 76 web tests passed; mdBook and link validation checked 64 pages and 103 rendered files |
...
| Required FlashMoe one-token smoke | Failed; quarantined from this llama.cpp/native-language tranche | On 2026-07-28, the rebuilt release binary again exited zero but printed `5` for `2+2=`; token `4` was second by 0.320473 logit. The repository gate therefore still blocks promotion of that FlashMoe checkpoint. Existing upstream-parity evidence classifies this as checkpoint/model quality rather than a collar failure; no output correction is permitted, and this result grants no FlashMoe parity claim |
...
| Phase 6 extended promotion evidence | Follow-on | Outer Qwen/DSML logical-prefix extraction profiling and additional property/fuzz corpora can promote stronger or broader prefix rules beyond the shipped conservative contract. They are not prerequisites for the qualified Rust/Python semantic tranche, whose exhaustive logical-prefix and rollback corpora pass |
...
| Phase 7 native Rust implementation | Passed v2 focused implementation gates | Exact-pinned rust-analyzer 0.0.344 loads one offline read world and, for exact profiles, reconstructs an independently writable world from its loaded inputs before inference. All 13 crate tests pass. They cover dependency/sysroot shape certainty; invalid import/type steering; target selection; cross-file rollback; identity/staleness; active-request refresh exclusion; zero-load source refresh; build-script/procedural-macro downgrade without execution; baseline debt; the promoted name/import, field/method, privacy, call/type, mutability, moved-from-reference, and trait-bound categories; unsupported relative-import downgrade; closure rejection; and a multi-file import repair after rollback. The root lifecycle tests cover cold preparation, warm reuse, independent invalid/valid final replay, in-memory source refresh, cross-stage process-cache reuse, and exact non-Rust target bypass. |
| Phase 7 native Python implementation | Passed qualified v1 profile | Exact-pinned Astral `ty` 0.0.6 loads and primes a frozen project/typeshed world before inference. Fifteen crate tests cover all six promoted diagnostic codes, `Any`/dynamic unknowns, baseline debt, generated suppression, rollback—including reconstruction of an earlier file after a later patch file has streamed—project closure, and the repairable-prefix rule. Five codes are explicitly closure-only; only the independently proven generated string-plus-integer literal can use `unsupported-operator` for statement-time hard rejection. Root lifecycle tests cover project-local and authorized external environments/editables, dependency identities and drift, single flight/cancellation, cache reuse, and independent final replay. Dynamic/import-hook/native environments and broader public-project corpora remain conservative future profiles, not missing guarantees from v1. |
| Phase 7 native Python lifecycle/resource qualifier | Passed v2 release matrix | Release-arm64 profile `a6093b4f…ac0bc` crossed cold readiness, warm request, exact process-cache reuse, invalid/valid replay, serialized-overlay stress, final recovery, peak RSS, and retained-growth barriers in separate processes. Tiny 4/7 measured 50 ms cold, 516 ms for 65 stress replays, 44.1 MB peak, and 2.6 MB retained growth; representative 1,024/515 measured 317 ms, 1.469 s/33, 59.3 MB, and 4.9 MB; large 10,000/5,003 measured 3.121 s, 4.126 s/9, 201.9 MB, and 14.5 MB. All 15,001 large-case modules were primed. The accepted ceilings are 60 s cold, 20 s warm/cache/replay, 120 s stress, 1 GiB peak, and 512 MiB retained growth |
| Phase 7 native Python semantic qualifier | Passed release matrix and streaming proof | Release-arm64 corpus `e073204a…0b40` ran 24 cases with 9 allows and 15 rejections through generation closure, independent final replay, and direct promoted-code replay. All decisions matched; all six promoted codes and four mutation tools are represented. It also passes 2,007 exhaustive UTF-8 prefix probes and 1,536 deterministic rollback/full-replay branches. Cold readiness was 42 ms; case-stage latency was 8 ms p50, 15 ms p95, and 16 ms maximum. The frozen dependency is a distilled surface, not a full upstream conformance claim |
...
| Phase 7 native Rust lifecycle/resource qualifier | Passed v2 release matrix | Release-arm64 profile `738a7cee…d42` prepared exact two-target worlds and crossed cold/warm/cache, invalid/valid replay, serialized-overlay stress, recovery, peak RSS, and retained-growth barriers. Tiny 6/6 measured 2.250 s cold, 148 ms for 65 stress replays, 873.0 MB peak, and 0.18 MB retained growth; representative 258/130 measured 2.302 s, 182 ms/33, 879.7 MB, and zero growth; large 2,050/1,026 measured 2.841 s, 638 ms/9, 939.0 MB, and 1.7 MB. All are below the 180 s cold, 20 s warm/cache, 30 s replay, 240 s stress, 4 GiB peak, and 512 MiB retained-growth ceilings |
...
| Phase 7 native Rust semantic qualifier | Passed v2 release matrix | Checked corpus `30a19114…ce904` runs 23 cases through production generation closure, independent execution replay, and direct native diagnostic/unknown replay. It covers all ten promoted diagnostic classes, positive and baseline-debt cases, cross-crate repair and unresolved-import transactions, conservative relative-import and create/delete topology unknowns, and all four mutation tools. All 10 allows and 13 rejections match across generation/final replay; 2,758 logical UTF-8 boundaries and 1,472 deterministic rollback/full-replay branches pass. Cold readiness was 2.183 s; stage latency was 6 ms p50, 11 ms p95, and 170 ms maximum |
| Phase 7 native Rust production qualification | Passed for exact v2 profile | The 13-test language crate, 23-case semantic corpus, v2 release lifecycle/resource matrix, and independent final replay all pass. A 4,097-sibling bounded-arena regression proves one candidate-batch analyzer checkpoint is reused rather than refreshed per vocabulary entry. Current local Qwen llama.cpp run `1785282623046-29977-0` processed 7,009 generated tokens across 13 calls without the former 4,096-checkpoint exhaustion or executing an invalid mutation; it stopped safely because the 7B model repeatedly chose a creation-only tool for an existing path. The earlier preserved run `1785221083352-60887-0` remains the successful strict edit/check/review/commit workflow proof. This qualifies the exact no-build-script/no-procedural-macro v2 profile, not arbitrary model competence, rustc parity, or FlashMoe backend quality |
| Phase 7 deeper/later language profiles | Pending | Rust macro/build-script/compiler parity, deeper `pb-control-python` environment/type matrices, and `pb-control-typescript` project matrices ship independently after their own soundness, lifecycle, multi-file, dependency, cancellation, latency, and final-replay evidence |
...
- **7C — Rust structural/HIR streaming (implemented).** Stream Rust syntax,
...
- **7D — Rust complete-transaction HIR v2 (implemented; semantic qualification passed in focused
  tests).** During
  readiness, clone the already loaded graph, roots, configuration, and VFS inputs into a separate
  writable Salsa storage rather than mutating a shared database clone. Overlay all completed
  modifications to indexed sources together; resolve supported imports against the candidate graph;
  query every local target module; compare promoted name, field/method, privacy, selected
  call/type/trait-bound, mutability, and moved-from-reference categories with baseline debt; then
  restore the baseline before unlocking. Later files can repair earlier files. Build-script and
  procedural-macro closures, create/delete topology, unsupported relative imports, complete
  ownership/borrow checking, trait-implementation completeness, and rustc parity remain later
  independently qualified profiles and must return `Unknown` meanwhile.
- **7E — Add `pb-control-python` (initial profile and semantic qualification implemented; release
  rerun pending).** The
  crate embeds exact-pinned Astral `ty` internals, a frozen in-memory project filesystem,
  independently writable pre-inference request databases, request-local create/modify/delete overlays,
  baseline diagnostic debt, generated-suppression rejection, and final transaction replay. Python
  3.12 fallback, the host target platform, first-party sources, bundled typeshed, configuration
  digests, dependency-manifest digests, and a separately snapshotted safe project-local
  virtual-environment image are in the current world contract. Captured dependency modules are
  primed before inference; ignored dependency bytes are rehashed after preparation and before
...
**Amended 2026-07-28:** Rust v2 adds a complete-transaction gate without turning an LSP diagnostic
stream into inference authority. Exact projects prepare an independently writable rust-analyzer
database before inference, apply all indexed-source candidates together only at tool closure, and
promote a closed diagnostic allowlist. The runtime still never executes build scripts or procedural
macros, and it does not claim full borrow checking, trait-implementation completeness, rustc parity,
or semantic authority for new/deleted source topology.

**Amended 2026-07-28:** the Python v1 layer follows the same independent promotion rule. Its native
...
| Native-world lifecycle budgets | Resolved for current Rust/Python profiles in Phase 7H | Isolated tiny/representative/large Python and Rust matrices now cover cold readiness, warm and exact-cache requests, invalid/valid final replay, sustained serialized-overlay contention, final recovery, peak RSS, and retained current-RSS growth. Source refresh, concurrent lease fallback, cancellation, and eviction retain focused regression coverage. Cross-host calibration and materially different project shapes remain future qualification profiles, not missing authority from the current profiles |
...
| Rust deeper native boundary | Phase 7D follow-on | The indexed-source complete-transaction HIR allowlist is implemented. Remaining promotion requires rust-analyzer/rustc parity corpora for build/procedural macros, relative module contexts, complete traits/borrows, source topology, `cfg`/target matrices, and compiler checks. |
