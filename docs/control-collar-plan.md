...
Status: **The syntax/canonical-patch collar is shipped. The first native Rust and Python semantic
layers, pre-inference lifecycle, streaming steering, independent execution replay, and checked
language-owned semantic/prefix qualifiers are implemented. Release lifecycle/resource/reclamation
and current live-backend qualification are the remaining production gates for this Rust/Python
tranche; TypeScript/JavaScript remains frozen at its shipped syntax level. Deeper compiler/runtime
coverage remains phased design-record work.** This is the durable design, migration history, and
evidence ledger. Curated architecture documentation describes only shipped guarantees.
...
| --- | --- | --- |
| Collar unit and chunk-boundary corpus | Passed | 69 deterministic tests, including all six language families plus TSX/JSX, byte/random chunk equivalence, monotonic hard rejection, source-length-only Rust branch rollback, transactional sibling-candidate restoration including rollback after tool closure, mixed-language finalization, viable split-UTF-8 Qwen/DSML boundaries, partial patch-line probing, embedded HTML languages, virtual write replacement, exact multi-hunk/create/delete patches including deletion events, malformed patch corpus, controller-bound and scalar-exact Qwen/DSML payload closure, immutable semantic identities/debt and receipts, and control-token identity |
...
| Workspace all-target tests | Passed | On 2026-07-28, 1,517 root tests passed with 25 device/environment/qualification tests ignored, 2 environment-contract tests passed, all 69 collar tests passed, all 15 native Python tests passed, and all 13 native Rust tests passed |
| Web and documentation tests | Passed | 76 web tests passed; mdBook and link validation checked 62 pages and 101 rendered files |
...
| Phase 7 native Rust latency/resource qualifier | Prior v1 baseline retained; v2 rerun required | On exact commit `23754bbe`, the earlier read-only profile measured an 87.289 s full-pb cold load, 1.014 s independent execution replay, and 1.007 s warm request. The prior instrumented run recorded process maximum RSS of 2,127,577,088 bytes and measured peak footprint of 129,778,360 bytes. Rust v2 intentionally adds a second writable Salsa storage during pre-inference readiness, so those numbers cannot qualify the new profile. Release cold/warm/replay time, peak memory, reclamation, and sustained serialized-overlay concurrency must be remeasured. |
...
| Phase 7 native Rust semantic qualifier | V2 focused matrix passed; release run pending | Checked corpus `30a19114…ce904` runs 23 cases through production generation closure, independent execution replay, and direct native diagnostic/unknown replay. It covers all ten promoted diagnostic classes, positive and baseline-debt cases, cross-crate repair and unresolved-import transactions, conservative relative-import and create/delete topology unknowns, and all four mutation tools. All decisions match across generation/final replay; 2,758 logical UTF-8 boundaries and 1,472 deterministic rollback/full-replay branches pass. The report is content-free and the offline ephemeral Cargo fixture executes no model, LSP, build script, procedural macro, or mutation. Optimized timing/resource evidence remains pending. |
| Phase 7 native Rust production qualification | V2 semantic gates passed; promotion blocked | The 13-test language-crate matrix and 23-case root semantic qualifier pass, including complete-overlay rejection, multi-file repair, exhaustive prefix probing, repeated rollback, baseline debt, and safe build/procedural-macro/topology downgrade. The existing root lifecycle and live workflow evidence predates the second writable database. V2 release resource/latency measurement, sustained serialization/reclamation, and a current live backend matrix remain. The required FlashMoe arithmetic smoke is also still nonsensical. |
...
| Native-world lifecycle budgets | Phase 7H | Python now has isolated tiny/representative/large cold, warm, exact-cache, final-replay, and peak-memory qualification. Rust large-project, per-language source refresh, concurrent lease fallback, eviction, sustained cancellation/reclamation, and cross-host budgets remain |
