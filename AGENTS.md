...
8. **Rust owns the web boundary**: Lifecycle state, project/session collections, usage totals and requested usage-window aggregates, terminal transitions, Trinity chatter, and API failures are server-authored. React mutation handlers may clear local form state after success, but must not synthesize domain or lifecycle state, refetch to discover a mutation result, or add polling to compensate for a missing server model. Extend the typed response or a revisioned SSE snapshot instead; every existing-session mutation response must use the same revisioned session projection as SSE, and a mutation that changes a streamed collection must return that collection's revisioned projection, so delayed or unavailable SSE cannot leave a control stale. Stream subscription floors, revision publication, retained transitions, and derived usage must share one causal boundary; derive totals from the captured session projection and keep any retained window or generation cache explicitly bounded. Any event field copied into persistence must update the live `SessionState` in the same locked publication before the event is visible; never maintain a sink-only shadow of live state. Collection rows may contain only fields whose changes publish that collection's revision—omit per-event counters that the collection does not consume. Each connection receives only terminal-transition deltas newer than its advancing cursor, only the live stream may authorize a new process generation, and every callback must prove it still belongs to the current browser stream instance. Loaded browser data remains usable during reconnect and must be distinguished from an initial request with no snapshot. A multi-await mutation transaction must be owned independently of the caller so disconnect or cancellation cannot split durable state, in-memory projections, accounting, and revision publication; a durable mutation should carry its exact committed projection into that transaction instead of relying on a second fallible read. A mutation must not return failure after its authoritative state has committed; represent incomplete ancillary cleanup as success with typed warnings. Every boundary change must keep Rust and TypeScript fields and enum values exact, update the browser runtime parser, and add a behavioral cross-language contract or regression test that would fail if the workaround returned.
9. **Protocol order is structural, not conventional**: Durable fields belong in `SessionRecord`; process handles and queues belong in `SessionRuntime`. Only the `AppState` session coordinator may insert, remove, persist, install, revise, or publish a session. Staging helpers may append only to isolated history, and `SessionEventSender` is a passive committed-publication channel. Every successful session transaction increments its independent session revision, including eventless changes, and publishes committed events followed by its exact snapshot. `SessionStatus` is the only lifecycle value in live state, persistence, lifecycle events, and session projections: never add `running`, `paused`, or another lifecycle mirror at any boundary. The coordinator compares status before and after every full transaction, appends `SessionStateChanged` itself, and derives dispatch whenever a transaction enters `Queued`; handlers must not request dispatch manually. Usage records are likewise canonical: the coordinator derives aggregate metrics and collection publication from their staged before/after value rather than trusting a second manually maintained total. When a lifecycle change enters `Completed` or `Failed`, the same coordinator derives the retained terminal-transition key. Separately, an active-to-inactive watch transition—including a safe pause without a pending question—publishes finish after collection publication and post-commit effects; handlers must not emit or label lifecycle/watch phases manually, and stream consumers must not poll lifecycle state. React must never compare the session revision with transcript sequence. Collection publication is derived from the before/after row projection plus committed usage/events rather than remembered by individual handlers. Runtime message sealing/opening, post-commit effects, and project-registry reconciliation remain inside an independently owned coordinator transaction. Every mutation adapter must carry that transaction's exact committed receipt; it may project a field for an established HTTP response but must never reconstruct or discard authoritative state or warnings. If reconciliation after an authoritative commit is incomplete, retain the typed warning in the revisioned collection until a successful retry clears it. Terminal RPC uses tagged frames: a unary request receives exactly one response or error, while watch receives one acknowledgement followed only by typed session-event, session-finished, replay-reset, or stream-error frames. The terminal client must reject non-contiguous live events, mismatched reset cursors/session IDs, response frames after acknowledgement, and EOF before `session_finished`. Add source-enforcement and failure/reordering regression tests whenever one of these invariants changes.
