...
export function IntegrationList({
  ...
  onUpgrade?: (item: InstalledIntegration) => void;
}) {
  const available = marketplace.filter((item) =>
    !isIntegrationInstalled(item, installed)
  );
  ...
  return (
    <div className="mcp-store-list" data-testid="mcp-store-list">
      {!hasItems
        ? (
          <div className="mcp-store-empty text-secondary small">
            {emptyText}
          </div>
        )
        : (
          <>
            {installed.map((item) => (
              <div
                key={`installed:${item.kind}:${item.name}`}
                className="mcp-store-row is-installed"
              >
                <div className="mcp-store-icon">
                  <i className={installedIcon}></i>
                </div>
                <div className="mcp-store-copy">
                  ...
                  <small>
                    {item.status === "legacy_unverified"
                      ...
                      ? `Ready · pinned ${
                        item.verified_manifest_digest.slice(0, 19)
                      }…`
            ...
            {available.map((item) => (
              <div key={`${item.kind}:${item.name}`} className="mcp-store-row">
                <div className="mcp-store-icon image-icon">
...
export function IntegrationConfigForm({
  ...
  useEffect(() => {
    ...
    for (const [key, property] of Object.entries(schema?.properties ?? {})) {
      if (pending.env?.[key] !== undefined) next[key] = pending.env[key];
        ...
        next[key] = String(property.default);
      }
    }
    setValues(next);
    setTouched({});
  ...
  const missingLspManifest = pending.kind === "lsp" &&
    !schemaResponse?.lsp_manifest;
  const canSubmit = !loading && !submitting && !missingLspManifest &&
    Object.keys(validationErrors).length === 0;

  return (
    <form
      className="integration-config-sheet"
      onSubmit={(event) => {
    ...
    >
      <div className="integration-config-head">
        <h3>
          {pending.operation === "upgrade" ? "Upgrade" : "Configure"}{" "}
          {pending.name || pending.containerImage}
        </h3>
        <button
          type="button"
      ...
      {loading && (
        <div className="alert alert-info py-2 small">
          ...
          container schema annotation…
        </div>
      )}
      {error && pending.kind !== "lsp" && (
        <div className="alert alert-warning py-2 small">
          Could not fetch the schema annotation, so this integration can be
          installed without extra fields. {error}
        </div>
      )}
      {error && pending.kind === "lsp" && (
        <div className="alert alert-danger py-2 small">
          Could not verify the package's typed LSP manifest, so installation is
          blocked. {error}
        </div>
      )}
      {!loading && !error && missingLspManifest && (
        <div className="alert alert-danger py-2 small">
          This image does not publish the typed LSP manifest required for
      ...
      {!loading && !error && !missingLspManifest && fields.length === 0 && (
        <div className="alert alert-secondary py-2 small">
          This container does not publish a configuration schema annotation, so
          it will be installed without extra environment values.
        </div>
      )}
      {schema?.description && (
        <p className="small text-secondary">{schema.description}</p>
      )}
          ...
          return (
            <div className="integration-config-field" key={key}>
              <label
                ...
                htmlFor={`integration-config-${key}`}
              >
                {label}
                ...
                : (
                  <input
                    ...
                    checked={type === "boolean"
                      ? values[key] === "true"
                      : undefined}
                    value={type === "boolean" ? undefined : values[key] || ""}
                    onBlur={() =>
                      setTouched((prev) => ({ ...prev, [key]: true }))}
                    onChange={(event) =>
                      setValues((prev) => ({
              ...
              {property.description && (
                <div className="form-text">{property.description}</div>
              )}
              {isInvalid && (
